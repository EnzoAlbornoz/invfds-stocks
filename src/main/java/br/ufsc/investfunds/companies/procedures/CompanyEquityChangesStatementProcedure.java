...
public class CompanyEquityChangesStatementProcedure {

    private final static Logger LOGGER = Logger.getLogger("CompanyEquityChangesStatementProcedure");

    public static void doRun(Connection conn, Path filePath) throws IOException, SQLException {
        // Load Query
        var query = new String(
                CompanyEquityChangesStatementProcedure.class.getResourceAsStream("/sql/insertDMPL.sql").readAllBytes(),
                "UTF-8");
        // Read from File
        ...
        try {
            ...
            for (var chunk: chunkRows) {
                ...
                for (var csvRow : chunk) {
                    ...
                    var statementColumn = csvRow.get("COLUNA_DF");
                    ...
                    stmt.setString(6, statementColumn);
                    stmt.setString(7, accountCode);
                    stmt.setString(8, accountDescription);
                    stmt.setBigDecimal(9, accountValue);
